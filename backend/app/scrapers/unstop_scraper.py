...
def scrape_unstop() -> List[Dict[str, Any]]:
    """
    Scrape hackathon data from Unstop (formerly Dare2Compete).
    ...
    try:
        ...
        hackathon_cards = soup.select(".opportunity-card")
        ...
        for card in hackathon_cards:
            try:
                ...
                name_elem = card.select_one(".opportunity-title")
                name = name_elem.text.strip() if name_elem else "Unknown Hackathon"
                
                link_elem = card.select_one("a.opportunity-card__link")
                registration_link = f"https://unstop.com{link_elem['href']}" if link_elem and 'href' in link_elem.attrs else ""
                
                location_elem = card.select_one(".location-name")
                location = location_elem.text.strip() if location_elem else None
                
                date_elem = card.select_one(".date-time-card")
                ...
                if date_text:
                    try:
                        ...
                        if len(date_parts) >= 2:
                            start_date = datetime.strptime(date_parts[0].strip(), "%d %b %Y")
                            end_date = datetime.strptime(date_parts[1].strip(), "%d %b %Y")
                    except Exception as e:
                        logger.warning(f"Failed to parse date: {date_text}. Error: {str(e)}")
                
                image_elem = card.select_one("img.opportunity-image")
        ...
        logger.error(f"Error scraping Unstop: {str(e)}")
        return []
